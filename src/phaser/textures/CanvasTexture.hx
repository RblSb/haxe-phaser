...
@:native("Phaser.Textures.CanvasTexture") extern class CanvasTexture extends Texture {
	...
	**/
	function draw(x:Float, y:Float, source:ts.AnyOf2<js.html.CanvasElement, js.html.ImageElement>, ?update:Bool):CanvasTexture;
	/**
		Draws the given texture frame to this CanvasTexture, then updates the internal
		ImageData buffer and arrays.
	**/
	function drawFrame(key:String, ?frame:ts.AnyOf2<String, Float>, ?x:Float, ?y:Float, ?update:Bool):CanvasTexture;
	/**
	...
	**/
	function getPixel(x:Float, y:Float, ?out:phaser.display.Color):phaser.display.Color;
	/**
		...
		otherwise this may return out of date color values, or worse - throw a run-time error as it tries to access an array element that doesn't exist.
	**/
	function getPixels(?x:Float, ?y:Float, ?width:Float, ?height:Float):Array<Array<phaser.types.textures.PixelConfig>>;
	/**
	...
	**/
	function clear(?x:Float, ?y:Float, ?width:Float, ?height:Float, ?update:Bool):CanvasTexture;
	/**
