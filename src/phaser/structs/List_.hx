...
@:native("Phaser.Structs.List") extern class List_<T> {
	...
	**/
	function add(child:Dynamic, ?skipCallback:Bool):Dynamic;
	/**
		Adds an item to list, starting at a specified index. Each item must be unique within the list.
	**/
	function addAt(child:ts.AnyOf2<Array<T>, T>, ?index:Float, ?skipCallback:Bool):ts.AnyOf2<Array<T>, T>;
	/**
	...
	**/
	function getAll(?property:String, ?value:Dynamic, ?startIndex:Float, ?endIndex:Float):Array<T>;
	/**
	...
	**/
	function moveAbove(child1:T, child2:T):Void;
	/**
		Moves the given array element below another one in the array.
	**/
	function moveBelow(child1:T, child2:T):Void;
	/**
		Removes one or many items from the List.
	**/
	function remove(child:Dynamic, ?skipCallback:Bool):Dynamic;
	/**
	...
	**/
	function each(callback:EachListCallback<T>, ?context:Dynamic, args:haxe.extern.Rest<Dynamic>):Void;
	/**
