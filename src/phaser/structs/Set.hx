...
@:native("Phaser.Structs.Set") extern class Set<T> {
	...
	**/
	function each(callback:EachSetCallback<T>, ?callbackScope:Dynamic):Set<T>;
	/**
		Passes each value in this Set to the given callback.
	...
	**/
	function iterate(callback:EachSetCallback<T>, ?callbackScope:Dynamic):Set<T>;
	/**
