...
@:native("Phaser.Geom.Mesh.Face") extern class Face {
	...
	**/
	function getInCenter(?local:Bool):phaser.math.Vector2;
	/**
	...
	**/
	function contains(x:Float, y:Float, ?calcMatrix:phaser.gameobjects.components.TransformMatrix):Bool;
	/**
	...
	**/
	function transformCoordinatesLocal(transformMatrix:phaser.math.Matrix4, width:Float, height:Float, cameraZ:Float):Face;
	/**
	...
	**/
	...
	/**
		...
		rectangle.
	**/
	function isInView(camera:phaser.cameras.scene2d.Camera, hideCCW:Bool, z:Float, alpha:Float, a:Float, b:Float, c:Float, d:Float, e:Float, f:Float, roundPixels:Bool):Bool;
	/**
		...
		are updated.
	**/
	function scrollUV(x:Float, y:Float):Face;
	/**
		...
		are updated.
	**/
	function scaleUV(x:Float, y:Float):Face;
	/**
		Sets the color value for each Vertex in this Face.
	**/
	function setColor(color:Float):Face;
	/**
		Calls the `Vertex.update` method on each of the vertices. This populates them
		with the new translated values, updating their `tx`, `ty` and `ta` properties.
	**/
	function update(alpha:Float, a:Float, b:Float, c:Float, d:Float, e:Float, f:Float, roundPixels:Bool):Face;
	/**
